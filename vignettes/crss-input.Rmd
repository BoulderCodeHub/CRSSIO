...
CRSS requires calendar year volume of natural flow for the St. Vrain river in Lyons, Colorado for each year of model simulation. The model uses this information to estimate the volume of transmountain diversions, where wetter years result in smaller transmountain diversion volumes and drier years result in greater transmountain diversions. 

The CRSSIO package uses the function st_vrain_nf_calc() to predict St. Vrain natural flow using total natural flow at Glenwood Springs and a linear regression developed by Reclamation.
...
First, convert the existing natural flow data from CoRiverNF to a Natural Flow Data (`nfd`) object. Also get the Sacramento Index data
and calculate St. Vrain natural flow using `st_vrain_nf_calc()`, which estimates St. Vrain natural flow using total natural flow
at Glenwood Springs and a regression developed by Reclamation.

For this example, we will assume we want to create CRSS input by applying ISM to the 2000-2019 natural flow record.
...
head(sac_yt)
st_vrain_nf =st_vrain_nf_calc(CoRiverNF::cyAnnTot$GlenwoodSprings)
head(st_vrain_nf )
...
Now, `nf` `sac_yt` and `st_vrain_nf ` can be combined into a "CRSS input" object (`crssi`):

```{r convertCRSSI}
crss_input <- crssi(
  flow = nf,
  sac_year_type = sac_yt,
  st_vrain_nf  = st_vrain_nf ,
...
Next, St. Vrain natural flow can be be calculated using `st_vrain_nf_calc()`. 

```{r StVrainCalc}
cy_nf=nf_to_annual(nf_to_total(rand_nf), year='cy', full_year = T, keep_monthly=F)
new_st_vrain_nf  <- st_vrain_nf_calc(co_tot_nf = cy_nf)
head(new_st_vrain_nf )
```

Now there are 5 years (water years) of natural flow and Sacramento Year Type index data. This just needs to be converted to a `crssi` object and then exported to the format expected by CRSS. Because the data are on a water year basis and CRSS works on calendar year basis, the conversion will result in only 4 years of CRSS input, i.e., October 2023 - September 2028 data will be trimmed to January 2024 - December 2027. Note there are also 4 calendar years of St. Vrain natural flow - these were already trimmed to 4 years because the calculation uses calendar years. 

```{r ex2-out}
crss_input2 <- crssi(
  flow = rand_nf,
  sac_year_type = new_sac_yt,
  st_vrain_nf  = new_st_vrain_nf ,
  scen_number = 99, 
