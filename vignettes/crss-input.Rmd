...
head(sac_yt)
StVrain_nf=st_vrain_nf_calc(CoRiverNF::cyAnnTot$GlenwoodSprings)
head(StVrain_nf)
...
Now, `nf` `sac_yt` and `StVrain_nf` can be combined into a "CRSS input" object (`crssi`):

```{r convertCRSSI}
crss_input <- crssi(
  flow = nf,
  sac_year_type = sac_yt,
  StVrain_nf = StVrain_nf,
...
new_StVrain_nf <- st_vrain_nf_calc(co_tot_nf = cy_nf)
head(new_StVrain_nf)
```
